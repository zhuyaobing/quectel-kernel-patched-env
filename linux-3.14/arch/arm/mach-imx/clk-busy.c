...
static int stub_clk_busy_divider_set_rate(struct clk_hw *hw, unsigned long rate,
					  unsigned long parent_rate)
{
	return 0;
}

static struct clk_ops stub_clk_busy_divider_ops = {
	.recalc_rate = clk_busy_divider_recalc_rate,
	.round_rate = clk_busy_divider_round_rate,
	.set_rate = stub_clk_busy_divider_set_rate,
};
#endif
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
	if (strcmp(name, "ahb") == 0)
		init.ops = &stub_clk_busy_divider_ops;
#endif
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
static int stub_clk_busy_mux_set_parent(struct clk_hw *hw, u8 index)
{
	return 0;
}

static struct clk_ops stub_clk_busy_mux_ops = {
	.get_parent = clk_busy_mux_get_parent,
	.set_parent = stub_clk_busy_mux_set_parent,
};
#endif
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
	if (strcmp(name, "periph") == 0)
		init.ops = &stub_clk_busy_mux_ops;
#endif

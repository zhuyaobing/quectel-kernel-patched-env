...
static int stub_clk_pllv3_prepare(struct clk_hw *hw)
{
	pr_debug("%s: Ignore clk_prepare\n", __clk_get_name(hw->clk));

	return 0;
}

static void stub_clk_pllv3_unprepare(struct clk_hw *hw)
{
	pr_debug("%s: Ignore clk_unprepare\n", __clk_get_name(hw->clk));
}

static int p4_clk_pllv3_enable(struct clk_hw *hw)
{
	pr_debug("%s: Ignore clk_enable\n", __clk_get_name(hw->clk));

	return 0;
}

static void p4_clk_pllv3_disable(struct clk_hw *hw)
{
	pr_debug("%s: Ignore clk_disable\n", __clk_get_name(hw->clk));
}

static int p4_clk_pllv3_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	pr_debug("%s: Ignore clk_set_rate\n", __clk_get_name(hw->clk));
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
static const struct clk_ops p4_clk_pllv3_bus_ops = {
	.enable		= p4_clk_pllv3_enable,
	.disable	= p4_clk_pllv3_disable,
	.recalc_rate	= clk_pllv3_recalc_rate,
	.round_rate	= clk_pllv3_round_rate,
	.set_rate	= p4_clk_pllv3_set_rate,
};
#endif

#ifdef CONFIG_P4_BSP_ARCH_MXC
static const struct clk_ops p4_clk_pllv3_usb_otg_ops = {
	.enable		= p4_clk_pllv3_enable,
	.disable	= p4_clk_pllv3_disable,
	.recalc_rate	= clk_pllv3_recalc_rate,
	.round_rate	= clk_pllv3_round_rate,
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
static const struct clk_ops p4_clk_pllv3_sys_ops = {
	.enable		= p4_clk_pllv3_enable,
	.disable	= p4_clk_pllv3_disable,
	.recalc_rate	= clk_pllv3_sys_recalc_rate,
	.round_rate	= clk_pllv3_sys_round_rate,
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
static const struct clk_ops stub_clk_pllv3_enet_ops = {
	.prepare	= stub_clk_pllv3_prepare,
	.unprepare	= stub_clk_pllv3_unprepare,
	.is_prepared	= clk_pllv3_is_prepared,
	.recalc_rate	= clk_pllv3_enet_recalc_rate,
};
#endif
			  ...
			  u32 div_mask)
{
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
		if (strcmp(name, "pll1") == 0)
			ops = &p4_clk_pllv3_sys_ops;
#endif
		break;
	case IMX_PLLV3_USB:
		ops = &clk_pllv3_ops;
#ifdef CONFIG_P4_BSP_ARCH_MXC
		if (strcmp(name, "pll3") == 0)
			ops = &p4_clk_pllv3_usb_otg_ops;
#endif
...
#ifdef CONFIG_P4_BSP_ARCH_MXC
		if (strcmp(name, "pll6") == 0)
			ops = &stub_clk_pllv3_enet_ops;
#endif
		break;
	...
	default:
		ops = &clk_pllv3_ops;
#ifdef CONFIG_P4_BSP_ARCH_MXC
		if (strcmp(name, "pll2") == 0)
			ops = &p4_clk_pllv3_bus_ops;
#endif
	}

/*
 *  linux/arch/p4/arch-arm/alignment.c
 *
 *  This file was copied from linux/arch/arm/mm/alignment.c and got some minor
 *  modifications to work on PikeOS.
 * 
...
/* KERNEL_THUMB2 not yet supported in P4Linux for ARM */
#define ARM(x...)       x
#define THUMB(x...)
...
#define get16t_unaligned_check(val,addr) \
	get_user(val, (unsigned short *)addr);
...
#define put16t_unaligned_check(val,addr) \
	put_user(val, (unsigned short *)addr)
...
do_alignment_finish_ldst(unsigned long addr, unsigned long instr, struct pt_regs *regs, union offset_union offset)
{
	...
	if (!LDST_P_BIT(instr) || LDST_W_BIT(instr))
		regs->context[RN_BITS(instr)] = addr;
}
...
do_alignment_ldrhstrh(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
	...
	if (LDST_L_BIT(instr)) {
		...
		put16_unaligned_check(regs->context[rd], addr);
 ...
 user:
	if (LDST_L_BIT(instr)) {
		...
		put16t_unaligned_check(regs->context[rd], addr);
		      ...
		      struct pt_regs *regs)
{
	...
	if (load) {
		...
		regs->context[rd2] = val;
	} else {
		put32_unaligned_check(regs->context[rd], addr);
		put32_unaligned_check(regs->context[rd2], addr + 4);
	}
 ...
 user:
	if (load) {
		...
		put32t_unaligned_check(regs->context[rd], addr);
		put32t_unaligned_check(regs->context[rd2], addr + 4);
	}
...
do_alignment_ldmstm(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
	...
	regs->context[HOST_EPC] += correction;
		...
		for (regbits = REGMASK_BITS(instr), rd = 0; regbits;
			...
			if (regbits & 1) {
				if (LDST_L_BIT(instr)) {
					...
					put32t_unaligned_check(regs->context[rd], eaddr);
				eaddr += 4;
			}
	} else {
		for (regbits = REGMASK_BITS(instr), rd = 0; regbits;
			...
			if (regbits & 1) {
				if (LDST_L_BIT(instr)) {
					...
					put32_unaligned_check(regs->context[rd], eaddr);
				eaddr += 4;
			}
	}

	if (LDST_W_BIT(instr))
		regs->context[rn] = newaddr;
...
do_alignment(unsigned long addr, unsigned int fsr, struct pt_regs *regs)
{
	...
	unsigned int type;
	mm_segment_t uninitialized_var(fs);
	unsigned int fault;
	u16 tinstr = 0;
	int isize = 4;
	int thumb2_32b = 0;
	...
	/*
	 * PikeOS __get_user does not work when KERNEL_DS is set and the address
	 * is in user space. The same is true when KERNEL_DS is not set and we
	 * are in kernel space.
	 */
	if (!user_mode(regs)) {
		fs = get_fs();
 ...
 fixup:
	...
	case 0x00000000:	/* 3.13.4 load/store instruction extensions */
		if (LDSTHD_I_BIT(instr))
			...
			offset.un = regs->context[RM_BITS(instr)];
	...
	case 0x06000000:	/* ldr or str register */
		...
		if (IS_SHIFT(instr)) {
			...
			case SHIFT_RORRRX:
				if (shiftval == 0) {
					offset.un >>= 1;
					if (regs->context[HOST_CPSR] & PSR_C_BIT)
						offset.un |= 1 << 31;
				} else
 ...
 bad_or_fault:
	 ...
	 */
	/*
	 * P4Linux: the calling function will take care when the alignment trap
 ...
 user:
	...
	if (ai_usermode & UM_SIGNAL) {
		 ...
		 */
		/* we doesn't support disabling the exception, just ignore it */
#if 0

...
/* re-define user_mode():
 * on PikeOS, we call these functions for user space contexts only
 */
#undef user_mode
...
static int emulate_dcbz(struct pt_regs *regs, unsigned char __user *addr)
{
	...
	size = L1_CACHE_BYTES;
#endif
	p = (long __user *) (regs->context[HOST_DAR] & -size);
			    ...
			    unsigned int flags, unsigned int instr)
{
	...
	if (flags & HARD) {
		if (flags & SX) {
			nb = regs->context[HOST_XER] & 127;
			if (nb == 0)
				return 1;
		} else {
			if (__get_user(instr,
				       (unsigned int __user *)regs->context[HOST_IP]))
				return -EFAULT;
			nb = (instr >> 11) & 0x1f;
			if (nb == 0)
				nb = 32;
...
int fix_alignment(struct pt_regs *regs)
{
	...
	addr = (unsigned char __user *)regs->context[HOST_DAR];
	...
	if (flags & LD) {
		...
		data.dd = *((double*)&regs->context[HOST_FR0+reg]);
	else
		data.ll = regs->context[reg];
	...
	if (flags & ST) {
		...
		*((double*)&regs->context[HOST_FR0+reg]) = data.dd;
	else
		regs->context[reg] = data.ll;

	/* Update RA as needed */
	if (flags & U)
		regs->context[areg] = regs->context[HOST_DAR];

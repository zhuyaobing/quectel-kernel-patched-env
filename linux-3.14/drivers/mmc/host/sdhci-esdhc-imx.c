			 ...
			 struct esdhc_platform_data *boarddata)
{
	...
	if (of_get_property(np, "omit-hsmode", NULL)) {
		host->quirks2 |= SDHCI_QUIRK2_CAPS_OMIT_HISPD;
		dev_info(mmc_dev(host->mmc), "preventing host from activating highspeed mode \n");
	}

				       ...
				       struct mmc_data *data)
{
	...
	if (data->host_cookie == COOKIE_MAPPED) {
		...
		if (data->flags & MMC_DATA_WRITE) {
			for_each_sg(data->sg, sg, data->sg_len, i) {
				uk_flush_dcache_range((unsigned long)phys_to_virt(sg->dma_address), (unsigned long)(sg->length));
			}
		}
#endif
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (caps[0] & SDHCI_CAN_DO_HISPD && !(host->quirks2 & SDHCI_QUIRK2_CAPS_OMIT_HISPD)) {
		pr_info("%s: setting high speed mode\n",mmc_hostname(mmc));

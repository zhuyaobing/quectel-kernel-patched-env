  ...
  while (utils_id_s)
    {
      if (utils_id_s->temp)
	{
	  int fd = fileno (utils_id_s->fp);
	  fclose (utils_id_s->fp);
	  errno = 0;
	  unlink (utils_id_s->name);
          if (errno != 0)
            fprintf (stderr, _("cannot remove %s: %s"), utils_id_s->name, strerror (errno));
	}

      utils_id_s = utils_id_s->link;
    }

...
static int cache_space_refresh(Server *s, JournalStorage *storage) {
        char a[FORMAT_BYTES_MAX], b[FORMAT_BYTES_MAX], c[FORMAT_BYTES_MAX], d[FORMAT_BYTES_MAX], e[FORMAT_BYTES_MAX], f[FORMAT_BYTES_MAX], g[FORMAT_BYTES_MAX], h[FORMAT_BYTES_MAX];
        JournalStorageSpace *space;
        JournalMetrics *metrics;
        uint64_t vfs_used, vfs_avail, avail, over_keep_free;
        usec_t ts;
        int r;
        ...
        /*
         * A journal that is persistent on a rootfs need to consider the increase
         * of disk-usage outside of the journal.
         *
         * Bug:
         *  The journal is not hard-limited to value provided in space->limit,
         *  meaning that it can grow over the limit with up to the maxfilesize
         *  configuration value, because only archived files are  used to
         *  calculate the usage). This has an impact on the vfs_used calculation
         *  done above. It seems that due to the schedule of vacuum, the used
         *  value will end up being max_file_size greater than the actual
         *  space->limit provided from the previous calculation step. The will
         *  lead to recalculation of a space->limit = vfs_used + avail (where
         *  avail is 0) which in turn continuously increases by max_file_size,
         *  eventually filling up the disk or hitting the hard-limit of max_use.
         *
         * Proposed solution:
         *  1. is any space available based on metrics->keep_free
         *  2. if 0 is return, either avail and keep_free are the same or
         *     keep_free is greater than avaible
         *  3. get how much vfs_avail is over metrics->keep_free (over_keep_free)
         *     3.1 if over_keep_free is 'le' than vfs_used, decrease available
         *         by the amount of over_keep_free
         *     3.2 else, if over_keep_free is greater than vfs_used, assume
         *         available is 0
         *  4. if there is still space avail compared to keep_free, set
         *     avail to currenly used (vfs_used) + what is left (avail)
         */

        avail = LESS_BY(vfs_avail, metrics->keep_free);
        if (avail == 0) {
                over_keep_free = LESS_BY(metrics->keep_free, vfs_avail);
                if (over_keep_free <= vfs_used)
                        avail = vfs_used - over_keep_free;
        } else {
                avail += vfs_used;
        }

        space->limit = MIN(MAX(avail, metrics->min_use), metrics->max_use);
        space->available = LESS_BY(space->limit, vfs_used);
        space->timestamp = ts;

        log_debug("space->vfs_used=%s space->vfs_available=%s avail=%s space->limit=%s space->available=%s metrics->keep_free=%s vfs_used=%s metrics->min_use=%s",
                  format_bytes(a, sizeof(a), space->vfs_used),
                  format_bytes(b, sizeof(b), space->vfs_available),
                  format_bytes(c, sizeof(c), avail),
                  format_bytes(d, sizeof(d), space->limit),
                  format_bytes(e, sizeof(e), space->available),
                  format_bytes(f, sizeof(f), metrics->keep_free),
                  format_bytes(g, sizeof(g), vfs_used),
                  format_bytes(h, sizeof(h), metrics->min_use));
